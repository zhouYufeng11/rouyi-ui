...
import { addSigapply, getSignature, uploadFiles } from '../../api/manage.js'
...
export default {
  ...
  created() {
    ...
    getSignature(this.formfile).then(res => {
      this.formfile.eAccount = res.rows[0].eaccount
      this.formfile.email = res.rows[0].email
      this.formfile.phone = res.rows[0].phone
      this.formfile.userId = res.rows[0].userId
      this.formfile.userName = res.rows[0].userName
    })
    getInfo().then(res => {
      // console.log(res)
      this.formfile.deptId = res.user.dept.deptId
    })
  },
  methods: {
    goback() {
    ...
    addList() {
      addSigapply(this.formfile).then(res => {
        // console.log(res)
