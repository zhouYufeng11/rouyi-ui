<template>
  <div class="downloadbox">
    <div class="boxtop">
      <span v-show="personInfo.applyType === 0">申请借阅</span>
      <span v-show="personInfo.applyType === 1">申请下载</span>
    </div>
    <div class="recordinp">
      ...
      <el-table
        ...
        class="tablebiao"
      >
        <el-table-column label="审批环节" show-overflow-tooltip width="150">
          <template slot-scope="scope">{{ scope.row.sort }}</template>
        </el-table-column>
        <el-table-column prop="status" label="意见类型" show-overflow-tooltip width="150">
          <template slot-scope="scope">
            <div class="button-group">
        ...
        <el-table-column prop="approverName" label="审批人" width="150" />
        <el-table-column prop="updateTime" label="审批时间" width="150" />
      </el-table>
      <el-select v-model="list2.status" placeholder="请选择" size="mini">
        <el-option
      ...
      <div class="tijiao">
        <el-button type="primary" class="button-style" size="small" @click="download">提交</el-button>
...
import { listManage, changeexamInfo, getApplyList, examList, abc, getdamanage } from '../../api/manage.js'
...
export default {
  ...
  created() {
    ...
    getApplyList(this.list).then(res => {
      ...
      this.list2.id = res.rows[0].id
      // console.log(this.daIds)
      // if (this.personInfo.certType === ) {}
    ...
    this.list3.daId = this.$route.query.id
    examList(this.list3).then(res => {
      // console.log(res)
      this.tablea = res.rows
      console.log(res)
    })
  ...
  methods: {
    ...
    download() {
      changeexamInfo(this.list2).then(res => {
        console.log(res)
        if (res.code === 200) {
          this.$message({
            message: '档案审核成功',
