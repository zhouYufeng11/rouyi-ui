...
import { applyList, addLookManage, eventList, getdamanage } from '../../api/manage.js'
...
export default {
  data() {
    return {
      ...
      tableData: [{
        a: '330000019-01-00190-001-2017-D5-0000001',
        b: '姜堰区素芳印刷厂申请“包装装潢印刷品和其他印刷品印刷企业主要登记事项变更(变更法定代表人(负责人)、经营场所)” 许可案',
        c: '姜堰区素芳印刷厂',
        d: '公开级',
        e: '30',
        f: '2017',
  ...
  created() {
    // this.daIds = (this.$route.query.id).split(',')
    if (this.$route.query.id) {
      for (let i = 0; i < (this.$route.query.id).split(',').length; i++) {
        if ((this.$route.query.id).split(',')[i]) {
          this.daIds.push((this.$route.query.id).split(',')[i])
        }
      }
  ...
  methods: {
    ...
    download() {
      this.personInfo.applyNum = this.daIds.length
      this.daApply.daApply = this.personInfo
      this.daApply.daIds = this.daIds
